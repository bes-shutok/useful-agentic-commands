...
•	Place knowledge at the correct scope (human, feature, Copilot, LLM example, or temporary).
...
•	Update copilot-instructions.md with high-quality, enforceable rules only.
...
Only include content in .github/docs/BEST_PRACTICES.md if all conditions are met:
	...
	•	not written to correct Copilot reasoning
...
•	LLM-only examples must not be embedded in copilot-instructions.md.

	8.	Copilot-only lessons (rules)

	•	Lessons whose purpose is to prevent Copilot from making incorrect assumptions must be encoded as rules in copilot-instructions.md.
	•	Copilot reasoning guidance must never appear in BEST_PRACTICES.

⸻

C. Copilot rule qualification gate (mandatory)

Before adding anything to copilot-instructions.md, apply all checks below.
...
F. Copilot instructions structure
19.	From communication lessons and documentation decisions, extract only enforceable rules.
20.	Update .github/copilot-instructions.md using this exact structure:
...
•	must not reference this repository, paths, or environment

Section 2: Repository style and conventions (look-alike guidelines)
•	rules for consistency in this repo only
•	examples:
