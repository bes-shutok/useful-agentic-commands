...
**Item 1, full-path OSError context.** `read_private_file` opens the final component dirfd-relative (`os.open(path.name, ..., dir_fd=parent_fd)`), so a non-translated `OSError` message surfaces only the bare file name.

Before (today): calling `read_private_file` on a missing file inside an existing directory raises `FileNotFoundError: [Errno 2] No such file or directory: 'absent.json'` with no directory context.
...
**Item 2, pinned sidecar reads.** `read_byte_buffer` (the reader the strict audit uses for sidecar content) still opens the full path string with only the target pre-check, so an attacker who can write a sidecar ancestor directory can swap a symlink into an ancestor and redirect the read.

Before (today): with `_reject_symlink` bypassed, `read_byte_buffer(anc_link / "target.json")` (where `anc_link` is a symlink to a real ancestor directory) follows the ancestor and returns the bytes.
...
**Item 3, window-aware chronicity.** Chronicity for the retry-delta (`retry_drops = unparseable_now - chronic`) is decided from the initial-buffer snapshot alone. A chronic sidecar repaired and then re-broken inside the same retry window ends unparseable (retry-induced final state) yet stays in `chronic`, so the operability stderr note stays silent even though the drop is retry-induced. Verified against today's tree by the executed probe.

Before (today): the probe scenario yields stderr with NO `dropped ... unparseable sidecar(s) (retry-induced)` note (probe output: stderr empty, rc 0, final disk unparseable).
