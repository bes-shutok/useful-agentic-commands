...
4. Every valid finding not fixed in this iteration has a durable backlog item (path recorded on the finding, in the address log when Step 3.3 launched, or in the skip-path disposition note); a finding held `pending` for the fix-risk user decision (Hard Gate 23) is recorded as returned-for-ask per review-staging's receiving-review consumer row, not backlogged.
