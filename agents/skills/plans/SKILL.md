...
**Decision points requiring a grill:** <point>: <one-line receipt: user decision, source, date>, or the literal line `Decision points requiring a grill: none remain.` when the ambiguity scan found no material decision point; when several points remain, their per-point receipts are carried into the plan's single Assumptions trailer line comma- or semicolon-separated, never one trailer line per point.
